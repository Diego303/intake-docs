...
This monitors the project directory with `watchfiles` and re-runs the checks from `acceptance.yaml` on each change. Useful during development to immediately see if the implementation meets the spec.
...
If you use intake's MCP server, AI agents can run verification programmatically with the `intake_verify` tool:
