...
```
[File changed] --> [Ignored file filtering] --> [Re-verification] --> [Terminal result]
                                    ^                                           |
...
Watch mode requires the optional dependency `watchfiles`:

```bash
pip install intake-ai-cli[watch]
```

If you try to run `intake watch` without having `watchfiles` installed, you will get an error message with installation instructions.
...
| `--verbose` / `-v` | `false` | Verbose output (enables debug logging). |
...
# With verbose output
intake watch specs/my-feature/ -p ./my-project -v
```
...
1. **Initial validation:** verifies that `SPEC_DIR` exists and contains `acceptance.yaml`. If not, throws `WatchError` with a suggestion.
...
4. **Debounce:** waits the configured interval after the last change before acting (default: 2 seconds). Prevents unnecessary re-runs during rapid edits.
5. **Ignored file filtering:** discards changes in files matching ignore patterns (`.git`, `__pycache__`, etc.).
...
- Checks are run and the result is displayed
...
By default, the following patterns are ignored (they do not trigger re-verification):
...
The filtering is exhaustive: the full path, the filename, and **each component** of the path are compared against each pattern. This means the `.git` pattern also ignores `.git/objects/abc`, `.git/refs/heads/main`, etc.
...
| `ignore_patterns` | list[string] | see above | Glob patterns for ignoring changes. |
...
intake verify specs/my-feature/ -p . -f junit > test-results.xml
...
intake watch specs/my-feature/ -p . --tags ci --debounce 3.0 &
WATCH_PID=$!
...
**Solution:** verify that the spec path is correct, or generate the spec first with `intake init`.
...
1. **The file matches an ignore pattern.** Check `watch.ignore_patterns` in `.intake.yaml`.
2. **The debounce is too high.** Reduce with `--debounce 1.0`.
3. **The file is outside the project directory.** Only the directory specified with `--project-dir` is watched.
