...
- **2 prompts** -- structured templates for implementation and correction flows
- **2 transports** -- stdio (local) and SSE (network)

The server allows an AI agent to access the complete spec, query pending tasks, update task status, run verification, and get correction suggestions -- all without leaving its workflow.
...
For SSE transport (HTTP), additional dependencies are needed:
...
# SSE (for remote or networked agents)
...
Runs the spec's verification checks. Returns the pass/fail result for each check.
...
Analyzes verification failures and suggests corrections. Internally runs verification, filters failed checks, and uses the feedback module to generate suggestions.
...
Resources allow direct access to a spec's files via URIs with the `intake://` scheme. Resources are auto-discovered: the server scans the specs directory and registers all available files.
...
The server provides 2 structured prompts that guide the agent in common workflows.
...
In `claude_desktop_config.json` or equivalent configuration:
...
6. Get correction suggestions with `intake_feedback`
...
```
Error: Spec 'my-feature' not found in ./specs
```
...
```bash
ls ./specs/
# Should show directories like: my-feature/
