---
title: "Deployment"
description: "Docker, pre-commit hooks, team installation and deployment patterns."
order: 14
...
docker run --rm \
  -v $(pwd):/workspace \
  intake verify /workspace/specs/my-feature/ -p /workspace

# Export for Claude Code
docker run --rm \
  -v $(pwd):/workspace \
  intake export /workspace/specs/my-feature/ -f claude-code -o /workspace
  ...
  - repo: local
    hooks:
      - id: intake-verify
        ...
        entry: intake verify specs/my-feature/ -p . --fail-fast
        language: system
...
```yaml
entry: intake verify specs/my-feature/ -p . --fail-fast -t structure -t security
```
...
See [CI/CD Integration](../ci-cd-integration/) for complete examples of GitHub Actions, GitLab CI, Jenkins, and Azure DevOps.
...
See [Security](../security/) for best practices on secret management.
