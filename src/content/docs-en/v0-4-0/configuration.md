...
| `stack` | list[string] | `[]` | Technology stack. If empty, auto-detected from project files. |
...
| Mode | Auto-detection criteria | Files generated |
|------|------------------------|----------------|
...
| `confluence.include_child_pages` | bool | `false` | Recursively include child pages. |
...
- **`minimal`**: Quick prototyping, exploratory ideas, a single developer. Low cost, minimal output.
...
If you don't have a `.intake.yaml`, intake uses sensible defaults. To create a basic configuration file:
