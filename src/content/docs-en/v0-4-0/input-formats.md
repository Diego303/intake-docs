...
intake supports 11 input formats through specialized parsers. The format is auto-detected by file extension and content. Parsers are discovered automatically via the [plugin system](../plugins/).
...
- **Full text**: content without the front matter
...
- Scanned PDFs (images only) are not supported directly -- use the image parser instead
...
- HTTP 4xx/5xx -> `ParseError` with status code
...
| `jira://PROJ?jql=sprint%20%3D%2042` | JQL query |
...
| `confluence://search?cql=space.key%3DENG` | CQL query |
...
| Empty files | Error | Empty files or those with only whitespace produce `EmptySourceError` |
...
The parser will be discovered automatically when the package is installed. See [Plugins](../plugins/) for details.
