...
description: "GitHub Actions, GitLab CI, Jenkins, Azure DevOps and pre-commit hooks."
order: 13
...
| Action | When | Command |
|--------|------|---------|
| Verify spec | On every PR / push | `intake verify specs/<spec>/ -p . -f junit` |
    ...
    steps:
      ...
      - uses: actions/setup-python@v5
        with:
          python-version: "3.12"
      - run: pip install intake-ai-cli
      - run: intake export specs/my-feature/ -f claude-code -o .
      - uses: actions/upload-artifact@v4
        with:
          name: exported-spec
          path: |
  ...
  check-drift:
    ...
    steps:
      ...
      - name: Check for stale specs
        id: drift
        run: |
          OUTPUT=$(intake show specs/my-feature/ 2>&1)
      ...
      - name: Create issue if drift detected
        ...
        run: |
          gh issue create \
            --title "Spec drift detected: my-feature" \
            --body "The spec sources have changed. Regenerate with: \`intake add specs/my-feature/ --regenerate\`" \
            --label "spec-drift"
        env:
...
pipeline {
    ...
    stages {
        ...
        stage('Verify') {
            steps {
                sh 'intake verify specs/my-feature/ -p . -f junit > test-results.xml'
            }
            post {
                always {
                    junit 'test-results.xml'
        ...
        stage('Export') {
            ...
            steps {
                sh 'intake export specs/my-feature/ -f claude-code -o .'
...
steps:
  ...
  - script: intake verify specs/my-feature/ -p . -f junit > test-results.xml
...
The most compatible format for CI. All systems support it:
...
```bash
intake verify specs/my-feature/ -p . -f json > results.json
```
...
repos:
  - repo: local
    hooks:
      - id: intake-verify
        name: Verify spec compliance
        entry: intake verify specs/my-feature/ -p . --fail-fast -t structure
        language: system
...
"Spec drift" occurs when requirement sources change but the spec is not regenerated. The `spec.lock.yaml` stores SHA-256 hashes of the original sources, enabling change detection.
...
```bash
# 1. intake show automatically detects staleness
intake show specs/my-feature/

# 2. If sources changed, regenerate
intake add specs/my-feature/ --regenerate

# 3. Review changes
intake diff specs/my-feature-old/ specs/my-feature/
```
...
- name: Notify Slack on failure
  ...
  run: |
    curl -X POST "${{ secrets.SLACK_WEBHOOK }}" \
      ...
      -d '{
        "text": "Spec verification failed for my-feature",
        "blocks": [
          {
            "type": "section",
            "text": {
  ...
  run: |
    curl -X POST "${{ secrets.TEAMS_WEBHOOK }}" \
      ...
      -d '{
        ...
        "text": "The my-feature spec did not pass acceptance checks on branch ${{ github.ref_name }}"
      }'
```
...
```bash
intake verify specs/my-feature/ -p . -t structure --fail-fast
```
