---
title: "Pipeline"
description: "How the 5-phase pipeline + feedback loop works in detail."
order: 5
...
- If the file is empty or contains only whitespace: `EmptySourceError` error
...
Before generating, source complexity is classified to select the optimal generation mode. This classification is heuristic (does not use the LLM).
...
The `AdaptiveSpecBuilder` wraps the standard `SpecBuilder` and filters files based on the mode:
...
Used to detect if sources changed since the last generation (`is_stale()`).
...
|------|-------|-------------|
| Tools | 7 | Operations on specs (show, verify, feedback, tasks, etc.) |
...
```
                    [ WATCH ]
                        |
    Detect changes --> Filter ignored --> Re-run verify --> Show result
         ^                                                        |
         └────────────────────────────────────────────────────────┘
```
