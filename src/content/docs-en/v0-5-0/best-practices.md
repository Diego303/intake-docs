...
Generated specs are text files -- ideal for version control with git:
...
The `spec.lock.yaml` has source hashes. You can check if sources changed:
...
The state is persisted directly in `tasks.md`, so it is versioned with git alongside the rest of the spec.

---

## Using sources from URLs
