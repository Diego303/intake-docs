...
Create a `.intake.yaml` file in the root of your project. Complete example:
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `jira.url` | string | `""` | Base URL of the Jira instance. Required to use `jira://`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `confluence.url` | string | `""` | Base URL of the Confluence instance. Required to use `confluence://`. |
...
MCP server (Model Context Protocol) configuration. See [MCP Server](../mcp-server/) for complete details.
...
**Note:** The MCP server requires the `mcp` package. Install with: `pip install "intake-ai-cli[mcp]"`. The SSE transport also requires `starlette` and `uvicorn`.
...
- **`minimal`**: Quick prototyping, exploratory ideas, single developer. Low cost, minimal output.
...
If you do not have a `.intake.yaml`, intake uses sensible defaults. To create a basic configuration file:
