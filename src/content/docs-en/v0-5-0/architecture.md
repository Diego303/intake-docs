...
│   ├── schema.py           #   11 config models (LLM, Project, Spec, Verification, Export, Security, Connectors, Feedback, MCP, Watch, Intake)
...
Everything that comes from the outside (`.intake.yaml` file, CLI flags) is validated with Pydantic v2 models. This ensures the configuration is correct before using it.
...
All extension points use `typing.Protocol` with `@runtime_checkable`, not abstract classes (ABC). This allows structural subtyping without inheritance:
...
Existing parsers (V1) continue to work. The registries accept both V1 and V2. To add a new parser, exporter or reporter, you only need to implement the correct interface — there is no need to inherit from any base class.
...
The registries (`ParserRegistry`, `ExporterRegistry`) try plugin discovery first and fall back to manual registration if it fails. This allows external plugins to register automatically just by installing the package.
...
The `analyze/complexity.py` module automatically classifies the complexity of sources to select the optimal generation mode:
