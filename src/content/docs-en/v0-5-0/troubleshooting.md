...
**Solution:** Verify that the file path is correct. Use relative paths to the current directory or absolute paths:
...
**Solution:** The `acceptance.yaml` file has YAML syntax errors. Verify:
...
### In what language is the spec generated?

By default in English (`en`). It is configured with `--lang` or `project.language`:
...
Yes, it is recommended. Specs are text files that benefit from version control. See [Spec versioning](../best-practices/#spec-versioning).
