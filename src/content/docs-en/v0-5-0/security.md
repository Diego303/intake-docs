---
title: "Security"
description: "Secret management, data redaction, offline mode and auditing."
order: 15
...
| spec.lock.yaml | Contains hashes and metadata of sources | Only partial hashes (16 hex chars), no content; safe to commit |
...
To verify that the patterns work correctly:

1. Run with `--dry-run` to see which sources would be processed
2. Review `spec.lock.yaml` to confirm which files were processed (only hashes, no content)
...
| Automatic data retention | No retention policies; files persist until deleted |
