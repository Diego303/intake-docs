...
- **Tasks** with dependencies (DAG), estimated time in minutes, files, checks
...
Before generating, classifies the complexity of sources to select the optimal generation mode. This classification is heuristic (does not use LLM).
...
Exposes specs as an MCP (Model Context Protocol) server so that any compatible AI agent can consume them programmatically. It is not a pipeline phase per se, but rather a consumption layer that sits after the export.
...
Monitors the project directory and automatically re-runs verification checks when it detects file changes. It complements the Verify phase with a continuous cycle during development.
...
Closes the loop between verification and implementation. When checks fail, it analyzes the causes and suggests fixes to both the implementation and the spec.
