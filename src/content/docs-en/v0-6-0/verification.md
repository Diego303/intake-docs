---
title: "Verification"
description: "Verification system to validate that code meets the spec."
order: 8
...
The verification engine executes acceptance checks defined in `acceptance.yaml` against the project directory. It answers the question: "does the implementation meet the spec?"
...
Executes a shell command and verifies that the exit code is 0.
...
The command is executed in the project directory (`--project-dir`). Stdout and stderr are captured (truncated to 1000 and 500 chars respectively).
...
Checks can have tags to execute subsets:
...
A check is executed if it has **at least one** of the specified tags.
...
The simplest way to integrate verification in CI. The official action installs intake, runs verify, and uploads the report as an artifact:
...
See [CI/CD Integration](../ci-cd-integration/#action-oficial-de-intake-recomendado) for the complete reference of inputs and outputs.
...
If you use the `generic` exporter, a standalone `verify.sh` is generated that does not need intake installed:
...
## Continuous verification with watch mode

To automatically re-execute checks every time you change a file in the project:

```bash
intake watch specs/my-spec/ -p .
```

This monitors the project directory with `watchfiles` and re-executes the `acceptance.yaml` checks on each change. Useful during development to immediately see if the implementation meets the spec.

```bash
# Only checks with tag "tests"
intake watch specs/my-spec/ -p . -t tests

# Longer debounce (5 seconds)
...
If you use the intake MCP server, AI agents can execute verification programmatically with the `intake_verify` tool:

```
Tool: intake_verify
Input: { "spec_name": "my-spec", "tags": ["tests"] }
Output: "Verification: 3/4 passed\n[PASS] check-01: Tests pass\n[FAIL] check-03: ..."
```

They can also use the `verify_and_fix` prompt for an automatic cycle of verify-fix-reverify.
...
| Code | Meaning |
|------|---------|
| `0` | All **required** checks passed (optional ones may have failed) |
