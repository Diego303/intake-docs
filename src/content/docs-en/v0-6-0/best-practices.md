...
Tips to get the most out of intake.
...
- Endpoint returns 201 on success, 409 if email already exists
```

### Separate functional from non-functional

intake distinguishes between functional requirements (what the system does) and non-functional requirements (how it does it). Separate them in sources for better extraction:
...
| GitHub Issues | Reported bugs, feature requests, backlog state |
...
**Recommendation:** Integrate `intake validate --strict` in your CI pipeline alongside `intake verify`.

---

## Estimate costs before generating

Use `intake estimate` to know how much generating a spec will cost before running `intake init`:
...
The `spec.lock.yaml` has hashes of the sources. You can verify if sources changed:
...
You can pass URLs directly as sources without manually downloading:
...
intake downloads the page, converts HTML to Markdown, and processes it like any other source. It auto-detects if it is Confluence, Jira, or GitHub content by URL patterns.
...
If you work with MCP-compatible AI agents (Claude Code, Claude Desktop, etc.), intake's MCP server allows the agent to directly access specs:
...
You can filter by tags to only run a subset of checks:
