---
title: "Configuration"
description: "Complete reference for the .intake.yaml file and environment variables."
order: 4
...
2. **Preset**: if `--preset` is used, applies a predefined set
...
Create a `.intake.yaml` file in your project root. Complete example:
...
project:
  ...
  language: en                    # Language of the generated content
...
connectors:
  ...
  confluence:
    ...
    max_depth: 1                  # Maximum depth of child pages
  github:
...
# Watch mode
watch:
  debounce_seconds: 2.0         # Seconds to wait after last change
...
# Cost estimation
estimate:
  tokens_per_word: 1.35         # Tokens/word ratio
...
Stack auto-detection searches for 28+ marker files in the project directory:
...
| `include_sources` | bool | `true` | Include `sources.md` with requirement-source traceability. |
...
Configuration for direct API connectors. They allow using URIs like `jira://PROJ-123` directly in `-s`. See [Connectors](../connectors/) for usage details.

**Jira:**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `jira.url` | string | `""` | Base URL of the Jira instance. Required for using `jira://`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `confluence.url` | string | `""` | Base URL of the Confluence instance. Required for using `confluence://`. |
...
| `confluence.max_depth` | int | `1` | Maximum depth of child pages. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `gitlab.url` | string | `"https://gitlab.com"` | GitLab instance URL. Required for using `gitlab://`. |
...
| `gitlab.include_comments` | bool | `true` | Include discussion notes in issues. |
...
MCP (Model Context Protocol) server configuration. See [MCP Server](../mcp-server/) for complete details.
...
**Note:** The MCP server requires the `mcp` package. Install with: `pip install "intake-ai-cli[mcp]"`. SSE transport also requires `starlette` and `uvicorn`.

### `watch` Section

Watch mode configuration for automatic re-verification. See [Watch Mode](../watch-mode/) for complete details.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `watch.debounce_seconds` | float | `2.0` | Wait time after the last file change before re-verifying. |
| `watch.ignore_patterns` | list[string] | `["*.pyc", "__pycache__", ".git", "node_modules", ".intake"]` | Glob patterns of files/directories to ignore. Compared against each path component. |
...
| `estimate.prompt_overhead_tokens` | int | `2000` | Additional tokens per LLM call (system prompt, formatting, etc.). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `templates.user_dir` | string | `".intake/templates"` | Project-relative directory to search for custom templates. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `redact_patterns` | list[string] | `[]` | Regex patterns to be removed from generated content. |
...
Presets are predefined configurations for common use cases. Apply them with `--preset`:
...
Variable names are configurable via `connectors.*.token_env` and `connectors.*.email_env` in `.intake.yaml`.

---

## Auto-generate Configuration

If you don't have `.intake.yaml`, intake uses sensible defaults. To create a basic configuration file:
