---
title: "Feedback Loop"
description: "Feedback loop to iteratively improve specs."
order: 11
...
The feedback loop closes the cycle between verification and implementation. When verification detects failed checks, the `feedback` module analyzes the causes and suggests fixes.
...
# Apply amendments to the spec automatically
...
1. If `--verify-report` is not provided, runs `intake verify` first to generate a JSON report
...
6. Displays suggestions in the terminal (or formats them for an agent)
...
The `--agent-format` flag formats suggestions for a specific agent:
...
feedback:
  ...
  include_code_snippets: true     # Include code snippets in suggestions
```
...
| `include_code_snippets` | bool | `true` | Asks the LLM to include code in suggestions. |
...
- `apply()` modifies spec files and returns counts
