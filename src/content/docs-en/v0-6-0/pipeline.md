---
title: "Pipeline"
description: "The 5 pipeline phases: Ingest, Analyze, Generate, Verify, Export."
order: 5
...
Takes the `ParsedContent` from all sources and uses the LLM to extract structured requirements, detect conflicts, evaluate risks, and produce a technical design.
...
The prompt is configured with: number of sources, language, requirement format (`ears`, `user-stories`, etc.).
...
- After each call, compares with `max_cost_per_spec`
- If the budget is exceeded, throws `CostLimitError` and the analysis stops
...
Before generating, the complexity of the sources is classified to select the optimal generation mode. This classification is heuristic (does not use LLM).
...
Executes checks defined in `acceptance.yaml` against the project directory. Produces a report with results.
...
Exposes specs as an MCP (Model Context Protocol) server so that any compatible AI agent can consume them programmatically. It is not a pipeline phase per se, but a consumption layer that sits after the export.
...
| Resources | 6 | Direct access to spec files via `intake://specs/{name}/{section}` URIs |
...
Monitors the project directory and automatically re-executes verification checks when it detects file changes. Complements the Verify phase with a continuous cycle during development.
...
```
                    [ WATCH ]
                        |
    Detect changes --> Filter ignored --> Re-execute verify --> Show result
         ^                                                          |
         └──────────────────────────────────────────────────────────┘
```
...
Closes the loop between verification and implementation. When checks fail, it analyzes the causes and suggests corrections to both the implementation and the spec.
