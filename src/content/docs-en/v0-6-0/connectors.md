---
title: "Connectors"
description: "Direct API connectors for Jira, Confluence, GitHub and GitLab."
order: 10
...
Data is saved as temporary JSON files compatible with `JiraParser`.
...
| `gitlab://group/project/issues/42,43,44` | Multiple issues separated by comma |
...
|-----------|-------------|---------|
| `labels` | Filter by labels (comma-separated) | `labels=bug,urgent` |
| `state` | Issue state | `state=opened`, `state=closed` |
| `milestone` | Filter by milestone | `milestone=v2.0` |
| `assignee_username` | Filter by assigned user | `assignee_username=jdoe` |
| `search` | Text search | `search=login` |
| `order_by` | Sort by field | `order_by=updated_at` |
| `sort` | Sort direction | `sort=asc` |
