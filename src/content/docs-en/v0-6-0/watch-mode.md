---
title: "Watch Mode"
description: "File monitoring with automatic spec regeneration."
order: 17
icon: "M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8zm11 3a3 3 0 100-6 3 3 0 000 6z"
---

Watch mode monitors project files and automatically re-runs verification checks when it detects changes. It uses `watchfiles` (Rust-based, efficient) for file observation with configurable debounce.

```
[File change] --> [Ignored filtering] --> [Re-verification] --> [Terminal result]
                          ^                                           |
...
5. **Ignored filtering:** discards changes in files that match ignore patterns (`.git`, `__pycache__`, etc.).
6. **Selective re-verification:** runs the relevant checks against the updated project.
7. **Results display:** shows a pass/fail summary with Rich formatting in the terminal.
...
| `.intake` | Internal intake directory |
...
class SpecWatcher:
    ...
    # Public methods
    run() -> None              # Start watching (blocks until Ctrl+C)
    run_once() -> VerificationReport  # Run verification once
...
# Or start continuous watching (blocks)
watcher.run()
```
...
Watch mode is designed for **local development**, not for CI/CD. In a CI pipeline, it is recommended to use `intake verify` directly:

```bash
# In CI: one-time verification
intake verify specs/my-feature/ -p . -f junit > test-results.xml

# In local development: continuous watching
...
1. Add additional ignore patterns in `.intake.yaml`:
   ...
   watch:
     ignore_patterns:
       ...
       - "*.egg-info"
   ```
2. Increase the debounce to reduce the re-verification frequency.
...
| `2` | Runtime error (spec not found, invalid config, watchfiles not installed). |
