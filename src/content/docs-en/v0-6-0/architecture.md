...
plugins/                  <- Plugin discovery via entry_points (PEP 621)
  |
ingest/                   <- PHASE 1: 12 parsers + plugin discovery. No LLM dependency.
  |
...
connectors/               <- 4 API connectors: Jira, Confluence, GitHub, GitLab (async fetch)
  |
feedback/                 <- Feedback loop: failure analysis + spec amendments (requires LLM)

mcp/                      <- MCP server: tools, resources, prompts (requires LLM for feedback)
watch/                    <- Watch mode: file monitoring + re-verification (uses verify/)
validate/                 <- Standalone: internal spec validation (quality gate, offline)
estimate/                 <- Standalone: LLM cost estimation (offline)
...
The modules `ingest/`, `generate/`, `verify/`, `export/`, `diff/`, `doctor/`, `validate/` and `estimate/` **never** import from `llm/` or `analyze/`. This guarantees that everything except `init`, `add` and `feedback` works offline.

Documented exceptions:
- `ImageParser` accepts an injected vision callable (does not import directly from the LLM)
- `feedback/` uses the LLM to analyze verification failures, but does not analyze requirements — it is an independent module from `analyze/`
...
│   ├── schema.py           #   15 config models (LLM, Project, Spec, Verification, Export, Security, Connectors, Feedback, MCP, Watch, Validate, Estimate, Templates, GitLab, Intake)
...
├── connectors/             # 4 direct API connectors (async fetch + plugin protocol)
...
│   └── gitlab_api.py        #   GitLab: issues, milestones, nested groups via python-gitlab
├── ingest/                 # Phase 1 — 12 parsers + registry + plugin discovery + auto-detection
...
│   ├── models.py            #   10 analysis pipeline dataclasses
...
│   ├── adaptive.py          #   AdaptiveSpecBuilder — file selection by mode
...
│   └── spec_updater.py      #   SpecUpdater: preview + apply amendments to spec
...
│   ├── resources.py        #   Dynamic resources: intake://specs/{name}/{section}
│   └── prompts.py          #   Templates: implement_next_task, verify_and_fix
├── watch/                  # Watch mode (monitoring + re-verification)
│   ├── __init__.py         #   WatchError exception
│   └── watcher.py          #   SpecWatcher with watchfiles (Rust-based)
├── templates/              # 17 Jinja2 templates (6 spec + 3 claude-code + 3 kiro + 1 cursor + 1 copilot + 1 feedback + 2 CI)
│   ├── requirements.md.j2
│   ├── design.md.j2
...
│   ├── feedback.md.j2       #   Feedback suggestion format
│   ├── gitlab_ci.yml.j2     #   CI template for GitLab
│   ├── github_actions.yml.j2 #  CI template for GitHub Actions
│   └── loader.py            #   TemplateLoader with user override support
├── validate/               # Internal spec validation (quality gate, offline)
│   └── checker.py           #   5 categories: structure, cross_reference, consistency, acceptance, completeness
├── estimate/               # LLM cost estimation (offline)
│   └── estimator.py         #   Pricing table, estimation by mode, budget warnings
└── utils/                  # Shared utilities
    ├── file_detect.py       #   Format detection by extension
    ├── project_detect.py    #   Auto-detection of tech stack
    ├── source_uri.py        #   URI parsing: file, stdin, url, jira://, github://, gitlab://
...
All data flowing through the pipeline uses standard library `dataclasses`. They are lightweight and don't need validation because the data has already been processed internally.

Examples: `ParsedContent`, `Requirement`, `Conflict`, `TaskItem`, `CheckResult`, `AnalysisResult`, `DesignResult`.

### Pydantic v2 — Configuration

Everything coming from the outside (`.intake.yaml` file, CLI flags) is validated with Pydantic v2 models. This guarantees that the configuration is correct before using it.

Examples: `IntakeConfig`, `LLMConfig`, `ProjectConfig`, `SpecConfig`.

**Rule:** They are never mixed. Config models don't appear inside pipeline data, and dataclasses don't validate user input.
...
V2 Protocols extend V1 with metadata and additional capabilities for external plugins:
...
Existing parsers (V1) continue to work. The registries accept both V1 and V2. To add a new parser, exporter or reporter, you just need to implement the correct interface — no need to inherit from any base class.
...
| `mcp` | >=1.0 | MCP server (tools, resources, prompts) |
| `python-gitlab` | >=4.0 | GitLab connector |
| `watchfiles` | >=1.0 | Watch mode (file monitoring, Rust-based) |
...
| Group | Content |
|-------|---------|
| `intake.parsers` | 12 built-in parsers |
| `intake.exporters` | 6 built-in exporters (2 V1 + 4 V2) |
| `intake.connectors` | 4 built-in connectors (Jira, Confluence, GitHub, GitLab) |
...
7. **Informative errors** — Each exception states what happened, why, and how to fix it.
