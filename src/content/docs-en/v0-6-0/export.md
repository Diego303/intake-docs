---
title: "Export"
description: "Export formats and output customization."
order: 9
...
The exporter adds (or replaces) an `## intake Spec` section in the existing `CLAUDE.md`. If it doesn't exist, it creates it. It includes:
...
|----------|-------------|
| `read_spec_file(path, filename)` | Reads a spec file (returns `""` if it doesn't exist) |
...
```bash
intake export-ci specs/my-feature/ -p gitlab    # Generates .gitlab-ci.yml
intake export-ci specs/my-feature/ -p github    # Generates .github/workflows/intake-verify.yml
```
