---
title: "Input Formats"
description: "The 11 supported parsers, what they extract and how they auto-detect."
order: 6
...
3. **JSON subtype**: if the extension is `.json`, the content is inspected in this order:
   ...
   - If it doesn't match any subtype -> `yaml` (treated as structured data)
...
**Note:** JSON subtype detection follows a strict priority order: Jira > GitHub Issues > Slack > generic YAML. This avoids ambiguities when a JSON has fields that could match multiple formats.
...
- **Sections by headings**: Word headings become structured sections
...
| `decision_count` | Decisions detected |
| `action_item_count` | Action items detected |
...
Issues are downloaded as temporary JSON and parsed with `JiraParser`. Comments are included based on `connectors.jira.include_comments`.
...
See [Configuration > Connectors](../configuration/) for complete configuration fields. Connectors need:
...
| Empty files | Error | Empty files or whitespace-only produce `EmptySourceError` |
...
The parser will be automatically discovered when installing the package. See [Plugins](../plugins/) for details.

Inheriting from a base class is not required — you just need to implement the correct interface (structural subtyping via `typing.Protocol`).
