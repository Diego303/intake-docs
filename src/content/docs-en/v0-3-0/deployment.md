...
In a project's `requirements.txt` or `pyproject.toml`:
...
# Verify a spec (does not need API key)
docker run --rm \
  -v $(pwd):/workspace \
...
The most common use case: verify specs on each PR and export on merge.
