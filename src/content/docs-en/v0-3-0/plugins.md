---
title: "Plugins"
description: "Plugin system: protocols, discovery, hooks and how to create external plugins."
order: 7
...
When intake loads a registry (`ParserRegistry`, `ExporterRegistry`), it attempts plugin discovery first. If discovery fails, it falls back to manual (hardcoded) registration.
...
intake includes 20 built-in plugins registered as entry_points in `pyproject.toml`:
...
You can implement either the V1 protocol (`Parser`) or V2 protocol (`ParserPlugin`). The registry accepts both.
...
asana = "my_plugin.parser:AsanaParser"
```
...
- Ready for wiring into future pipeline phases
