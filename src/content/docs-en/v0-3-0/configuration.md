...
intake works without configuration — it only needs an LLM API key. To customize behavior, use a `.intake.yaml` file at the project root.
...
Create a `.intake.yaml` file at your project root. Complete example:
...
connectors:
  ...
  confluence:
    ...
    max_depth: 1                  # Maximum depth for child pages
  github:
...
| `fine` | Small and atomic tasks. Each task is ~15-30 minutes of work. |
...
| `confluence.max_depth` | int | `1` | Maximum depth for child pages. |
