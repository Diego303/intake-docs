---
title: "Export"
description: "6 export formats for AI agents."
order: 9
...
A self-contained bash script that runs the `command` checks from `acceptance.yaml`. Works the same as the Generic one but is located in `.intake/`.
...
A Markdown file with structured instructions for Copilot:
...
|----------|-------------|
| `read_spec_file(path, filename)` | Reads a spec file (returns `""` if not found) |
...
In addition to agent formats, intake can generate CI configuration for automatic verification:

```bash
intake export-ci specs/mi-feature/ -p gitlab    # Generates .gitlab-ci.yml
intake export-ci specs/mi-feature/ -p github    # Generates .github/workflows/intake-verify.yml
```

See [CLI Guide > export-ci](../cli-guide/#intake-export-ci) and [CI/CD Integration](../ci-cd-integration/) for more details.
