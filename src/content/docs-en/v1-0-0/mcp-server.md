...
The server allows an AI agent to access the full spec, query pending tasks, update task status, run verification, and get fix suggestions — all without leaving its workflow.
...
Gets the `context.md` content of a spec.
...
|-----------|----------|---------|-------------|
| `spec_name` | Yes | — | Name of the spec |
| `strict` | No | `false` | Strict mode: warnings become errors |
...
Estimates the LLM cost for generating or regenerating a spec based on existing files.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `spec_name` | Yes | Name of the spec |

Scans the spec files to count words and estimates tokens, number of LLM calls, and cost in dollars.
...
|-------|------|---------|-------------|
| `specs_dir` | string | `"./specs"` | Directory where the specs generated by intake reside |
...
Commit `.mcp.json` to the repo so the entire team gets automatic access to the MCP server.
...
| `mcp/tools.py` | Definition and handlers for the 9 tools |
| `mcp/resources.py` | Resource listing and reading (spec files) |
