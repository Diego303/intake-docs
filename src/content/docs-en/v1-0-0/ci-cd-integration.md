...
description: "GitHub Actions, GitLab CI, Jenkins, Azure DevOps."
order: 13
...
Complete guide to integrating intake into continuous integration pipelines. Includes examples for GitHub Actions, GitLab CI, Jenkins, and Azure DevOps.
...
| Validate spec | On every PR / push | `intake validate specs/<spec>/` |
| Export spec | When merging to main | `intake export specs/<spec>/ -f <format> -o .` |
...
The most CI-compatible format. All systems support it:
...
- **Weekly** (cron job): detect drift from sources that change without regenerating
...
# 2. If the sources changed, regenerate
...
**Note:** `export-ci` uses built-in Jinja2 templates (`gitlab_ci.yml.j2`, `github_actions.yml.j2`). If you need to customize them, you can override them with [user templates](../configuration/#templates-section).
...
Caching pip reduces ~30 seconds per run:
...
For quick checks in pre-commit, run only structure checks (without tests):
