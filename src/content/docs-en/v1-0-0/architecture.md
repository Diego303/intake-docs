...
- `ImageParser` accepts an injected vision callable (does not import directly from LLM)
...
All data flowing through the pipeline uses standard library `dataclasses`. They are lightweight and do not need validation because the data has already been processed internally.

Examples: `ParsedContent`, `Requirement`, `Conflict`, `TaskItem`, `CheckResult`, `AnalysisResult`, `DesignResult`.

### Pydantic v2 -- Configuration

Everything that comes from the outside (`.intake.yaml` file, CLI flags) is validated with Pydantic v2 models. This ensures the configuration is correct before use.
...
| `sources.md` | `sources.md.j2` | Sources used, requirement-to-source mapping, conflict sources |
...
See [Plugins](../plugins/) for full documentation.
...
The `AdaptiveSpecBuilder` wraps the standard `SpecBuilder` and filters files according to the mode. A specific mode can be forced with `--mode` in the CLI.
...
7. **Informative errors** -- Every exception states what happened, why, and how to fix it.
