---
title: "Watch Mode"
description: "Watch mode: file monitoring and automatic re-verification."
order: 17
...
If you try to run `intake watch` without having `watchfiles` installed, you will get an error message with the installation instructions.
...
**Note:** the CLI `--debounce` flag overrides the `debounce_seconds` value in the configuration.
...
1. **The file is in an ignore pattern.** Review `watch.ignore_patterns` in `.intake.yaml`.
...
1. Add additional ignore patterns in `.intake.yaml`:
   ...
   watch:
     ignore_patterns:
       ...
       - "*.egg-info"
   ```
2. Increase debounce to reduce re-verification frequency.
